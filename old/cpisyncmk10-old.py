...
################QI2 Defines###################
#QI2_EXPO SE when set to high requests exposure
    ...
    def pollQi2(self):
        ...
        while True:
            ...
            if newval[0] != oldval[0]:
                if newval[0] == 1:
                    #readyOn = time.clock()
                    print str(datetime.now())[:-3], 'Qi2 END EXPOSURE'
                    #print 'expose off to ready on', readyOn - expoEndTime
                    #print 'expose out on to ready on', readyOn - expoStartTime
                    #print 'total ready OFF time:', readyOn-readyOff
            if newval[1] != oldval[1]:
                # change occured
                if newval[1] == 1:
                    #expoStartTime=time.clock()
                    #gap = expoStartTime - expoEndTime
                    #print 'Gap:', gap
                    print str(datetime.now())[:-3], 'Qi2 START EXPOSURE'
                    #print 'Trigger to start', expoStartTime -self.niTriggerTime
                else:
                    #maybe good time to reset nikon trigger
                    self.write('NikonTrigger',0)
                    #expoEndTime= time.clock()
                    #expoLength=expoEndTime - expoStartTime
                    #print 'Exposure length:', expoLength
                    #print 'added:', expoLength + gap
                    #print str(datetime.now())[:-3], 'Qi2 STOP EXPOSURE'
                    #print 'Trigger to exposeoutend', expoEndTime -self.niTriggerTime
            oldval = newval
            self.updatePVs()
            #f.write(str(time.clock()-temptest)+'\n')
            time.sleep(.001)
            #self.busy_wait(0.0005)
            #poll(evt=1.e-6, iot=0.1)
    ...
    def pollCPI(self):
        ...
        while True:
            ...
            if newval[2] != oldval[2]:
                # change occured
                if newval[2] == 1:
                    print str(datetime.now())[:-3], 'GENERATOR RAD ENABLE'
                else:
                    if self.getParam("PhotospotIn")==1:
                        self.write("PhotospotIn", 0)
                    print str(datetime.now())[:-3], 'GENERATOR RAD END'
