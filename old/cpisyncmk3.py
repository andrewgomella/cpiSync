...
#Function to convert error code to text string
...
class myDriver(Driver):
    def  __init__(self):
        ...
        # (Section of code below was deprecated due to the nikon shutter delay)
        ...
        #set up change detection as part of DI rad prep line
        ...
        #DAQmxStartTask(self.radReadyOutTask)      
        ...
        #DAQmxStartTask(self.exposeOutTask)      
        """
        #Declare variables for change detection
        self.multipleOld=numpy.array([0,0,0], dtype=numpy.uint8)
        self.multipleNew=numpy.array([0,0,0], dtype=numpy.uint8)

        self.exposeOutVal= numpy.zeros((1,), dtype=numpy.uint8)
        self.radPrepOutVal= numpy.zeros((1,), dtype=numpy.uint8)
        self.radReadyOutVal= numpy.zeros((1,), dtype=numpy.uint8)
        self.exposeOutValNew= numpy.zeros((1,), dtype=numpy.uint8)
        self.radPrepOutValNew= numpy.zeros((1,), dtype=numpy.uint8)
        self.radReadyOutValNew= numpy.zeros((1,), dtype=numpy.uint8)
        #Initialize variables
        #DAQmxReadDigitalLines(self.radReadyOutTask, 1, 1, 0,  self.radReadyOutVal, 1, None, None, None)
        #DAQmxReadDigitalLines(self.radPrepOutTask,  1, 1, 0,  self.radPrepOutVal, 1, None, None, None)
        #DAQmxReadDigitalLines(self.exposeOutTask,   1, 1, 0,  self.exposeOutVal, 1, None, None, None)
        #prep-ready-expose

    def DIChangeCallback(self, Task, status, callbackData): 
        print 'callback'
        startcallback=time.clock()
        DAQmxReadDigitalLines(self.radPrepOutTask, 1, 1, 0,  self.multipleNew, 3, None, None, None)
        print self.multipleNew[0], self.multipleNew[1], self.multipleNew[2]
        #print 'readmultiple', time.clock()-startcallback
        #print 'multiple', self.multiple
        #print 'readlines', time.clock()-startcallback
        if  self.multipleNew[2] != self.multipleOld[2]:
            print 'expose out changed'
            if  self.multipleNew[2] == 1:
        ...
        #print 'cb 1st chunk', time.clock()-startcallback
        if self.multipleNew[0] != self.multipleOld[0]:
            print 'radprepout changed'
            if self.multipleNew[0] == 1:
                self.setParam("RadPrepOut", 1)
                self.PrepSound = True
            else:
                self.setParam("RadPrepOut", 0)
                self.PrepSound = False
        #print 'cb 2nd chunk', time.clock()-startcallback
        if self.multipleNew[1] != self.multipleOld[1]:
            print 'radreadyout changed'
            if self.multipleNew[1] == 1:
                self.setParam("RadReadyOut", 1)
            else:
                self.setParam("RadReadyOut", 0)
        print  self.multipleOld, self.multipleNew
        self.updatePVs()
        #print 'callback function took', time.clock()-startcallback
