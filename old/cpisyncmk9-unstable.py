...
class myDriver(Driver):
    def  __init__(self):
        ...
        #clocks
        self.genDelay=0 #time how long it takes generator to expose after request
        self.camDelay=0 #time how long it takes qi2 to expose after request
        self.genStart=0
        self.qi2Start=0
    ...
    def pollQi2(self):
        ...
        while True:
            ...
            if newval[1] != oldval[1]:
                # change occured
                if newval[1] == 1:
                    self.qi2Start = time.clock()
                    print str(datetime.now())[:-3], 'Qi2 EXP', ' Req to Exp:', self.qi2Start - self.camDelay, ' Gen to Exp:', self.qi2Start - self.genStart
                else:
                    print str(datetime.now())[:-3], 'Qi2 END', ' Exp High for:',  time.clock() - self.qi2Start, ' Req to Exp Low:', time.clock()-self.camDelay
                oldval = newval
            self.updatePVs()
    ...
    def DIChangeCallback(self, Task, status, callbackData): 
        ...
        DAQmxReadDigitalLines(Task, 1, 1, 0,  multipleNew, 3, None, None, None)
        if  multipleNew[2] != self.multipleOld[2]:
            if  multipleNew[2] == 1:
                #print  "Expose out is now high"
                self.setParam("ExposeOut", 1)
                self.genStart = time.clock()
                print str(datetime.now())[:-3], 'CPI EXP', ' Req to Exp:', self.genStart - self.genDelay
            else:
                print str(datetime.now())[:-3], 'CPI END', ' Exp High for:', time.clock() - self.genDelay
    ...
    def exposeNow(self):
        if self.cancel == 0:
            ...
            #temp = time.clock()
            # delay because we know the qi2 takes 17 millisec to start output
            # the cpi usually takes 10-15 milliseconds 
            # so if we delay 7 milliseconds we have highest chance of lining up with cpi
            poll(evt=.007, iot=0.1) # MUCH more accurate
            #print 'slept for ', time.clock() - temp
            ...
            while self.getParam("Expose") == 1 or self.getParam("Qi2Exposing") == 1:
                ...
                if self.cancel == 1:
                    break
            print 'Delay between gen/qi2 start:', self.genStart - self.qi2Start 
