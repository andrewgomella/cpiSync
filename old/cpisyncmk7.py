...
FILENAME_PV=PV('Qi2:TIFF1:FileName')
...
class myDriver(Driver):
    ...
    def pollQi2(self):
        ...
        if oldval[0] == 1:
            self.setParam("Qi2TriggerReady", 1)
        elif oldval[0] == 0:
            self.setParam("Qi2TriggerReady", 0)
        if oldval[1] == 1:
            self.setParam("Qi2Exposing", 1)
        elif oldval[1] == 0:
            self.setParam("Qi2Exposing", 0)
        self.updatePVs()
        while True:
            DAQmxReadDigitalLines(self.qi2PollTask, 1, 1, 0,  newval, 2, None, None, None)    
            if oldval[0] != newval[0] or oldval[1] != newval[1]:
                if newval[0] == 1:
                    self.setParam("Qi2TriggerReady", 1)
                elif newval[0] == 0:
                    self.setParam("Qi2TriggerReady", 0)
    ...
    def checkIfCameraReady(self):
        if self.cancel == 0:
            #For Qi2 this is extremely simple. If TriggeReady is high, it is ready to expose
            if self.getParam('Qi2TriggerReady') !=1:
                time.sleep(POLL_TIME)
            ##############################D800#########################################
            #Check NikonBuffer:
    ...
    def TriggerNikon(self):
        x=DAQmxWriteDigitalLines(self.Qi2TriggerTask,1,1,10.0,DAQmx_Val_GroupByChannel,self.one,self.written, None)
        self.processDAQstatus(x)

    def TriggerNikonReset(self):
        x=DAQmxWriteDigitalLines(self.Qi2TriggerTask,1,1,10.0,DAQmx_Val_GroupByChannel,self.zero,self.written, None)
        self.processDAQstatus(x)
