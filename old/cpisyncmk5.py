...
class myDriver(Driver):
    ...
    def document(self):
        print str(datetime.now()),'Generate Doc String Start'
        ...
        try:
            ...
            print str(datetime.now()),'Generate Doc String Finish'
        except:
            print str(datetime.now()), "Error writing doc string", Exception
    ...
    def NikonExpose(self):
        ...
        if ScanStatus == 1: #scan is in progress
            ...
            print str(datetime.now()), "Scanstatus", ScanStatus, "Nfinished", Nfinished, "Ntotal", Ntotal, #"NfinishedIOC", NfinishedIOC
            if self.getParam("RadReadyOut")==0: #the scan just started if this is true
                #grab info for doc string once
        ...
        if self.cancel == 0: #go ahead with exposure
            ...
            #reset nikon reload timer
            #   for sample scans, this reload time may already be reached due to 
            #   waiting for the motor to complete its move
            #   however, if that happens very quickly, we may need to wait to 
            #   to ensure that the nikon is ready for the next shot
            print str(datetime.now()), 'wait for nikon reload', time.time()-self.NikonReloadTimer
            while time.time()-self.NikonReloadTimer < NIKON_RELOAD_TIME:
                time.sleep(.001)
            exposestarttime=time.time()
            print 'time from NikonExpose function start to expose actually starting', time.time()-exposestart
            ...
            self.write('Expose', 1)
            #to save time generate doc string during exposure
            if D800_DOC.get() == 1:
                self.document()
            while self.getParam("Expose") == 1 and self.cancel == 0:
                #print str(datetime.now()), "wait for expose off"
        ...
        if self.cancel == 0:
            stopexpose=time.time()
            print str(datetime.now()), 'stopping exposure outputs'
            print 'time from starting exposure outputs to stopping them', time.time()-exposestarttime
            ...
            #self.ExposeOff() #handled by callback through nidaq change detection
            self.setParam('NikonBuffer', self.getParam('NikonBuffer')+1 ) #image incoming to add to buffer count
        ...
        self.updatePVs()
        print 'time from stopping exposure outputs to NikonExpose function end', time.time()-stopexpose
        print 'entire NikonExpose Sequence took:', time.time()-exposestart
